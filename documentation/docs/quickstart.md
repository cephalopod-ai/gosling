  ...
  <TabItem value="windows" label="Windows">
    ...
    <Tabs groupId="interface">
      ...
      <TabItem value="cli" label="gosling CLI">
        ...
        If you see a PATH warning after installation, you'll need to add gosling to your PATH before running `gosling configure`. See the [Windows CLI installation instructions](/docs/getting-started/installation) for detailed steps.
...
gosling works with [supported LLM providers](/docs/getting-started/providers) that give gosling the AI intelligence it needs to understand your requests. On first use, you'll be prompted to configure your preferred provider.
...
<Tabs groupId="interface">
  ...
  gosling will open a browser for you to authenticate with Tetrate, or create a new account if you don't have one already. When you return to the gosling desktop app, you're ready to begin your first session.
...
## Start Session
Sessions are single, continuous conversations between you and gosling. Let's start one.

<Tabs groupId="interface">
    <TabItem value="ui" label="gosling Desktop" default>
        After choosing an LLM provider, click the `Home` button in the sidebar.

        Type your questions, tasks, or instructions directly into the input field, and gosling will immediately get to work.
    </TabItem>
    <TabItem value="cli" label="gosling CLI">
        1. Make an empty directory (e.g. `gosling-demo`) and navigate to that directory from the terminal.
...
From the prompt, you can interact with gosling by typing your instructions exactly as you would speak to a developer.

Let's ask gosling to make a tic-tac-toe game!

```
create an interactive browser-based tic-tac-toe game in javascript where a player competes against a bot
```

gosling will create a plan and then get right to work on implementing it. Once done, your directory should contain a JavaScript file as well as an HTML page for playing.


## Enable an Extension

While you're able to manually navigate to your working directory and open the HTML file in a browser, wouldn't it be better if gosling did that for you? Let's give gosling the ability to open a web browser by enabling the [`Computer Controller` extension](/docs/mcp/computer-controller-mcp).
...
<Tabs groupId="interface">
    ...
    <TabItem value="ui" label="gosling Desktop" default>
        ...
        5. Now that gosling has browser capabilities, let's ask it to launch your game in a browser:
    </TabItem>
    <TabItem value="cli" label="gosling CLI">
        ...
        ```
        4. Now that gosling has browser capabilities, let's resume your last session:
        ```sh
         gosling session -r
        ```
        5. Ask gosling to launch your game in a browser:
    </TabItem>
</Tabs>
...
## Next Steps
Congrats, you've successfully used gosling to develop a web app! 🎉

Here are some ideas for next steps:
* Continue your session with gosling and improve your game (styling, functionality, etc).
* Browse other available [extensions](/extensions) and install more to enhance gosling's functionality even further.
* Provide gosling with a [set of hints](/docs/guides/context-engineering/using-goslinghints) to use within your sessions.
* See how you can set up [access controls](/docs/mcp/developer-mcp#configuring-access-controls) if you don't want gosling to work autonomously.
