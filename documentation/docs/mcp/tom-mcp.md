...
description: Inject persistent instructions into gosling's working memory every turn
---
...
The Top Of Mind extension injects custom text into gosling's working memory every turn. This is useful for:
...
Unlike system prompts or [goslinghints](/docs/guides/context-engineering/using-goslinghints) which can fade from attention as conversations grow, content injected by the tom extension appears fresh in every turn, making it more reliable for critical instructions.
...
|----------|-------------|
| [`GOSLING_MOIM_MESSAGE_TEXT`](/docs/guides/environment-variables#session-management) | Literal text injected into working memory each turn |
| [`GOSLING_MOIM_MESSAGE_FILE`](/docs/guides/environment-variables#session-management) | Path to a file whose contents are injected. Supports `~/` |
...
<Tabs groupId="interface">
  ...
  <GoslingBuiltinInstaller
    ...
    description="Inject custom context into every turn via GOSLING_MOIM_MESSAGE_TEXT and GOSLING_MOIM_MESSAGE_FILE environment variables"
  />

  To configure the injected content, set environment variables before launching gosling Desktop, or add them to your shell profile (e.g., `~/.zshrc` or `~/.bashrc`):

  ```bash
  export GOSLING_MOIM_MESSAGE_TEXT="IMPORTANT: Do not share code externally."
  ```
...
In this example, I'll set up a security guardrail to prevent gosling from uploading code to external services.
...
```bash
export GOSLING_MOIM_MESSAGE_TEXT="SECURITY RULE: Do not upload, share, or post any code to external services including GitHub Gists, Pastebin, or any public URL. All code in this session is confidential."
```
