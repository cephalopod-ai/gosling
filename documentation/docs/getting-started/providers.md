...
| [ChatGPT Codex](https://chatgpt.com/codex) | Access GPT-6 Astra and GPT-5 models through an eligible ChatGPT account. **Requires a ChatGPT Plus/Pro subscription.** | No manual key. Uses browser-based OAuth authentication for both CLI and Desktop. |
...
| [OpenAI](https://platform.openai.com/api-keys)                              | Provides GPT-6 Astra (`gpt-6-astra`), gpt-4o, o1, and other advanced language models. gosling routes Astra tool calls through the Responses API and supports its `low` through `max` reasoning levels. Also supports OpenAI-compatible endpoints (e.g., self-hosted LLaMA, vLLM, KServe). **o1-mini and o1-preview are not supported because gosling uses tool calling.** | `OPENAI_API_KEY`, `OPENAI_HOST` (optional), `OPENAI_ORGANIZATION` (optional), `OPENAI_PROJECT` (optional), `OPENAI_CUSTOM_HEADERS` (optional)                                       |
...
| [xAI](https://x.ai/)                                                        | Access to xAI's Grok models including grok-4.7, grok-4.6, and grok-4.20 with up to a 1,000,000 token context window.                                                                                                            | `XAI_API_KEY`, `XAI_HOST` (optional)                                                                                                                                                |
