...
gosling can autonomously decide to use subagents when it determines they would be beneficial for your task - you don't always need to explicitly request them. This happens automatically in autonomous [permission mode](/docs/guides/managing-tools/gosling-permissions) (the default). Subagents are disabled in manual approval, smart approval, and chat-only modes.
:::

To use subagents, ask gosling to delegate tasks using natural language. gosling automatically decides when to spawn subagents and handles their lifecycle. You can:

1. **Request specialized help**: "Use a code reviewer to analyze this function for security issues"
2. **Run parallel tasks**: "Create three HTML templates simultaneously"
3. **Delegate complex work**: "Research quantum computing developments and summarize findings"
4. **Control extension access**: "Create a subagent with only the developer extension to refactor the code"
...
When gosling delegates work to a subagent, you can see the subagent's tool calls in real-time. This transparency helps you understand what the subagent is doing and verify it's on the right track.

<Tabs groupId="interface">
  <TabItem value="desktop" label="gosling Desktop" default>
...
Internal subagents spawn gosling instances to handle tasks using your current session's context and extensions. You configure and execute internal subagents with direct prompts.
...
External subagents let you bring in AI agents from other providers and platforms, enabling gosling to coordinate and integrate your workflow with the broader ecosystem. In the below example, we use Codex as a subagent by running it as an MCP server:

**[gosling Configuration File](/docs/guides/config-files)** (`~/.config/gosling/config.yaml`):
```yaml
subagent:
  args:
  - mcp-server
...
1. **Core Agent System** (`crates/gosling/src/agents/`)
   - Agent orchestration and session management
   - Tool execution framework
   - Extension system integration

2. **CLI Interface** (`crates/gosling-cli/`)
   - Command-line interface and session handling
   - Configuration management

3. **ACP Server** (`crates/gosling/src/acp/`)
   - ACP session and custom-request handling
   - HTTP/WebSocket transport for Desktop and external clients
...
|-----------|---------|------------------|
| **Max Turns** | 25 | Use natural language or set `GOSLING_SUBAGENT_MAX_TURNS` |
...
**Environment variable:** Set `GOSLING_SUBAGENT_MAX_TURNS` to change the default max turns for all subagents.
...
The `delegate` and `load` tools are provided by the `summon` platform extension, which is enabled by default.
:::
...
### Return Mode Control
Choose how much information gosling provides from its subagents in your main session.
...
- **Extension tools**: Use tools from extensions inherited from the parent session
  ...
  items={[
    {
      ...
      description: 'Deep dive into gosling subagents. Walk through building an app using 6 specialized AI agents for advanced workflow automation and development.',
    ...
    },
    {
      ...
      thumbnailUrl: 'https://gosling-docs.ai/assets/images/tutorial-using-subagents-ef265627024db73e73d80e5799ed0c1a.png',
    ...
    },
    {
      ...
      description: 'Discover how to choose the right tools for efficient task execution in gosling. Learn which approach is best for your workflow.',
