...
- PC-001 reopens Gates 5–8 for REQ-025. The extension adds a shared artifact-routing type and
  resolver, an `ArtifactRouterContext`, Electron save/copy/download IPC, an artifact-pane Save Copy
  action, migration of workspace/session exports, focused resolver/context/main-path/component
  tests, an entry-point negative-space audit, and updated acceptance evidence. Existing app-update,
  configuration, and automatic session-archive destinations stay outside product-output routing.
  Completed with ADR-0006, six repaired audit findings, 502 Desktop tests, documentation checks,
  workspace Rust tests, formatting, and mandatory clippy; see `evidence/pc-001.md`.
