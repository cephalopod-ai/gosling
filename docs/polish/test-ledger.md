...
| Rust core baseline | source bin/activate-hermit && cargo test -p gosling --lib | passed: 1,753 tests on 2026-08-27 | Core library behavior passed before this campaign's changes. | Post-change full validation is recorded in the campaign log. |
...
| Rust campaign build/tests/lint | source bin/activate-hermit && cargo build && cargo test -p gosling -p gosling-mcp -p gosling-cli && cargo clippy --all-targets -- -D warnings | passed on 2026-08-27 | Build, 1,886 gosling library tests plus integration/doc tests, 252 CLI tests, 88 MCP tests, and all-target Clippy are clean. | Platform-specific remote jobs remain separate evidence. |
| Desktop campaign validation | source bin/activate-hermit && cd ui/desktop && pnpm run typecheck && pnpm test -- --run | passed: 133 files, 1,069 tests | Desktop types and unit/integration behavior, including chat scrolling and updater surfaces, pass. | No packaged release artifact. |
