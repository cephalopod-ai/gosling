...
| TODO-20260817-005 | partial | P1 | Release hygiene | Run approved secret/history and dependency-audit tooling. | security redaction ledger | Rust advisories and unused dependencies pass `cargo-deny` and `cargo-machete`; documentation npm audit is recorded; dedicated secret/history scanners remain unavailable. |
| TODO-20260827-004 | closed | P0 | Release automation | Make BuildNotify safe when its optional Discord webhook is absent. | build-notify.yml | Missing secret exits successfully with an explicit notice; configured webhooks retain the existing failure payload. |
| TODO-20260827-005 | external-gate | P0 | Repository protection | Enable and verify the discovered default-branch ruleset. | GitHub ruleset 18782969 | Ruleset is enabled and its required checks are verified on a pushed revision. |
| TODO-20260827-006 | external-gate | P0 | Publication | Sign, tag, publish, and read back the v1.1.0 release only after every blocking checklist item passes. | RELEASE_CHECKLIST.md | Installed artifacts, signatures, checksums, remote CI, tag, release assets, and unauthenticated readback are observed. |
