...
- Preconditions: fixture with a small declared context limit; ordered-marker histories covering both more and fewer than the preferred protected-turn count; one short session whose first real turn contains multiple large completed tool pairs; ability to set the reduction value via CLI env var and via the ACP/Desktop preference.
- Steps: run with the default reduction (0.15) and cross the auto-compact threshold in each history shape; record the Desktop `est` value and compaction start/target/result notices; set the value to `0` and repeat; set an invalid env var value (negative, `1.0`, non-numeric) and observe the CLI warning and fallback; set the ACP preference to valid boundary values (`0`, just under `1`) and to invalid ones (negative, `1.0`, non-numeric) and confirm read/save versus rejection; run manual `/compact` and a hard-limit recovery compaction while a non-default reduction is configured; relaunch and confirm the saved preference persists.
- Expected: a non-zero reduction selects only the oldest safe prefix whose raw size satisfies the threshold-minus-reduction budget, even with fewer than the preferred protected turns; completed tool request/response pairs stay atomic and a large tool loop does not force the whole turn into the summary; the completion notice reports the actual resulting estimate, including summary overhead; `0` fully collapses the eligible region exactly as before; invalid settings are rejected or disable auto-compaction as documented; manual `/compact` and hard-limit recovery keep full-collapse behavior; the persisted preference survives relaunch and maps to the same `GOSLING_AUTO_COMPACT_REDUCTION` config key the CLI reads.
- Observe: the active estimate that triggered compaction, raw-context target, resulting estimate, compaction request count, and which messages or completed tool pairs end up summarized versus left verbatim.
