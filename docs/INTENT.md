...
| REQ-031 | P0       | Provide inventory-backed workspace provider, model, and thinking-effort defaults.    | The workspace editor uses accessible provider/model selectors, preserves an optional app/provider default, preselects ChatGPT Codex `gpt-5.6-terra` with medium effort for new drafts, limits effort choices to the selected model's advertised capability, persists the choice canonically, and pins it into new sessions without changing global settings. |
| REQ-032 | P0       | Start new workspace drafts in `~/Work` and support additional working folders.       | The renderer receives the platform home directory from the main process, initializes a new draft at `<home>/Work`, and lets users choose, relink, reveal, and remove multiple read/write folders explicitly classified as working folders. Existing workspaces retain their saved paths. |
| REQ-033 | P0       | Let users choose a workspace profile at new-chat submission.                         | The Hub displays an accessible workspace selector, preselects the active/default workspace for compatibility, blocks unavailable workspaces, and passes the selected workspace ID and primary folder through the shared create-session seam. Selecting a profile does not mutate backend active state or an existing session. |
...
| Active workspace       | Sidebar/filter selection and initial choice shown in the new-chat workspace picker. | The workspace pinned to the visible session.               |
