...
| REQ-025 | P1 | Default applicable application save/export paths to product outputs. | Each existing product artifact save/export entry point discovered during implementation chooses the matching/default workspace output; if no such central entry point exists, this is documented with agent context still provided. |
...
1. REQ-025 application-level output defaults if no centralized product save/export seam exists.
