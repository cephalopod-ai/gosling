...
function repositoryRoot(profilePath, options = {}) {
  if (options.repositoryRoot) {
    let configured;
    try {
      configured = fs.realpathSync(options.repositoryRoot);
    } catch {
      fail('trusted profile repository root does not exist');
    }
    if (!isContained(configured, profilePath)) {
      fail('profile is outside the trusted repository root');
    }
    return configured;
  }
...
function approvedRoots(root, configuredRoots) {
  if (configuredRoots) {
    return configuredRoots.map((entry) => {
      const candidate = path.resolve(root, entry);
      if (!isContained(root, candidate)) fail('trusted approved profile root escapes repository');
      try {
        return fs.realpathSync(candidate);
      } catch {
        fail('trusted approved profile root does not exist');
      }
    });
  }
...
function assertApproved(root, candidate, label, configuredRoots) {
  if (!approvedRoots(root, configuredRoots).some((approved) => isContained(approved, candidate))) {
...
function containedPath(root, candidate, label, kind, requireNonSymlink = false, configuredRoots) {
...
function assetInventory(root, assetRoot, iconBase, targets, configuredRoots) {
...
function validateProfile(raw, profilePath, options = {}) {
  exactKeys(
    raw,
    [
      'schemaVersion',
  ...
  const root = repositoryRoot(profilePath, options);
  assertApproved(root, profilePath, 'profile', options.approvedRoots);
  ...
  const assetsByTarget = assetInventory(
    root,
    assetRoot,
    assets.iconBase,
    assets.requiredTargets,
    options.approvedRoots
  );
...
function resolveProfile(profileFile, options = {}) {
  let profilePath;
  try {
  ...
  const paths = validateProfile(raw, profilePath, options);
  ...
  const head = options.goslingRevision ?? currentRevision(paths.root);
  if (!/^[0-9a-f]{40}$/.test(head)) fail('trusted Gosling revision is invalid');
  ...
  return {
    ...
    sourceClean: options.sourceClean ?? checkoutIsClean(paths.root),
    profilePath,
