...
2. Before reading any peer result, start one asynchronous \`delegate\` task for each non-lead roster member. Use that member's exact \`provider\` and \`model\`. Give every delegate the complete research brief, the relevant Initial Input contents or stable locators, the expected report schema, and instructions to research independently.${extensionArgument}
