...
2. Before reading any peer result, start one asynchronous \`delegate\` task for each non-lead roster member. Use that member's exact \`provider\` and \`model\`. Give every delegate the complete research brief${initialInputBrief}, the expected report schema, and instructions to research independently. This is an ad-hoc task. Use the payload shape \`{"instructions":"…","provider":"<exact provider>","model":"<exact model>","async":true}\`; omit the \`source\` key entirely instead of sending it as empty, null, \`dummy\`, or any other placeholder.${extensionArgument} Validate this exact payload before launch. Do not retry a rejected launch; record that roster seat as failed and continue degraded.
