...
export function registerSettingsIpcHandlers(
  ...
  targetIpcMain.handle(desktopCommandChannels.getSetting, (_event, key: unknown) => {
    if (!isSettingKey(key)) throw new Error('Invalid setting key');
    return rendererSettingValue(getSettings(), key);
  });
  targetIpcMain.handle(desktopCommandChannels.getSettings, (_event, keys: unknown) => {
  ...
  targetIpcMain.handle(desktopCommandChannels.getResearchLibraryPath, async (event) =>
    ensureResearchLibrary(event.sender.id)
  );
  targetIpcMain.handle(desktopCommandChannels.chooseResearchLibraryPath, async (event) => {
