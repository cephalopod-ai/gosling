...
BACKUP_DIR="${HOME}/.local/share/gosling/gosling-db-backups"
...
MIGRATIONS_DIR="${HOME}/.local/share/gosling/migrations"
RUST_SESSION_MANAGER="crates/gosling/src/session/session_manager.rs"
...
get_gosling_db_path() {
    if [[ -n "${GOSLING_PATH_ROOT:-}" ]]; then
        echo "${GOSLING_PATH_ROOT}/data/sessions/sessions.db"
    else
        local possible_paths=(
            "${HOME}/.local/share/gosling/sessions/sessions.db"
            "${HOME}/Library/Application Support/Block/gosling/data/sessions/sessions.db"
        )
...
show_status() {
    echo -e "${BLUE}=== Gosling Database Status ===${NC}"
    ...
    if [[ ! -f "${DB_PATH}" ]]; then
        ...
        echo ""
        echo "This is normal if you haven't run Gosling yet."
        echo "Once you run Gosling, a database will be created automatically."
        return
    fi
...
generate_migrations() {
    if [[ ! -f "${RUST_SESSION_MANAGER}" ]]; then
        ...
        echo "Make sure you're running this from the gosling repository root."
        exit 1
    fi
...
show_help() {
    ...
    echo -e "${BLUE}Gosling Database Migration Helper${NC}"
    echo ""
    ...
    echo "    rm -rf ~/.local/share/gosling/migrations/004_*"
    echo "    $0 generate-migrations"
    echo ""
