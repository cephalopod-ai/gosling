...
A native desktop app for macOS, Linux, and Windows. A full CLI for terminal workflows. An ACP interface and SDK for integrations. Built in Rust for portability.
...
The Rust core owns agent execution, provider contracts, permissions, session persistence, and MCP integration. Electron and terminal interfaces use those shared contracts through ACP rather than maintaining separate agent or REST-server behavior. See the [architecture overview](docs/architecture.md) and [documentation architecture section](documentation/docs/gosling-architecture/) for deeper design material.
...
- CLI command names are renamed from goose's. Use `gosling` and `gosling serve`; the former standalone `goslingd` binary and duplicate REST server crate were removed. Scripts that call `goose`, `goosed`, or `goslingd` need updating.
