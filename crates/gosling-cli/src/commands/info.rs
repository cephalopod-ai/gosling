...
async fn check_provider(
    ...
    let (provider, model) = match (config.get_gosling_provider(), config.get_gosling_model()) {
    ...
    let model_config = gosling::model_config::model_config_from_user_config(&provider, &model)
        .map_err(|e| ProviderCheckError::InvalidModel(e.to_string()))?;

    let provider_client = gosling::providers::create(&provider, Vec::new())
        .await
        .map_err(|e| {
    ...
    gosling::session_context::with_session_id(
...
pub async fn handle_info(verbose: bool, check: bool) -> Result<()> {
    ...
    println!("{}", style("gosling Version:").cyan().bold());
    ...
    if check {
        ...
        // checks, health probes) can rely on `gosling info --check` as a
        // pre-flight verifier.
        if result.is_err() {
