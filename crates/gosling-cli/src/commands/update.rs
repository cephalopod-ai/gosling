...
fn asset_name() -> &'static str {
    ...
    {
        "gosling-aarch64-apple-darwin.tar.bz2"
    }
    #[cfg(all(target_os = "macos", target_arch = "x86_64"))]
    {
        "gosling-x86_64-apple-darwin.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "gnu"))]
    {
        "gosling-x86_64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "gnu"))]
    {
        "gosling-aarch64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "musl"))]
    {
        "gosling-x86_64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "musl"))]
    {
        "gosling-aarch64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64"))]
    {
        "gosling-x86_64-pc-windows-msvc.zip"
    }
}
...
fn sha256_hex(data: &[u8]) -> String {
    ...
    hasher.update(data);
    gosling::utils::bytes_to_hex(hasher.finalize())
}
...
async fn fetch_attestations(digest: &str, token: Option<&str>) -> Result<Vec<serde_json::Value>> {
    let url = format!(
        "https://api.github.com/repos/repo-makeover/gosling/attestations/sha256:{digest}\
...
/// Update the gosling binary to the latest release.
///
...
pub async fn update(canary: bool, reconfigure: bool) -> Result<()> {
    ...
    #[cfg(not(feature = "disable-update"))]
    {
        ...
        let url =
            format!("https://github.com/repo-makeover/gosling/releases/download/{tag}/{asset}");
        ...
        println!("gosling updated successfully (verified with Sigstore SLSA provenance).");

        // --- Reconfigure if requested -------------------------------------------
        if reconfigure {
            println!("Running gosling configure...");
            let status = Command::new(current_exe)
                .arg("configure")
                .status()
                .context("Failed to run gosling configure")?;
            if !status.success() {
                eprintln!("Warning: gosling configure exited with {status}");
            }
        }
...
///   1. A `gosling-package/` subdirectory (Windows releases)
...
fn find_binary(extract_dir: &Path, binary_name: &str) -> Option<PathBuf> {
    // 1. Check gosling-package subdir (matches download_cli.sh / download_cli.ps1)
    let package_dir = extract_dir.join("gosling-package");
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if old_exe.exists() {
            fs::remove_file(&old_exe).with_context(|| {
                format!(
                    "Failed to remove old backup {}. Is another gosling process running?",
                    old_exe.display()
                )
            })?;
        }
        ...
        fs::rename(current_exe, &old_exe).with_context(|| {
            format!(
                "Failed to rename running binary to {}. Try closing Gosling Desktop if it's open.",
                old_exe.display()
            )
        })?;
...
mod tests {
    ...
    fn test_asset_name_valid() {
        ...
        assert!(name.starts_with("gosling-"));
        #[cfg(target_os = "windows")]
    ...
    fn test_binary_name() {
        ...
        #[cfg(target_os = "windows")]
        assert_eq!(name, "gosling.exe");
        #[cfg(not(target_os = "windows"))]
        assert_eq!(name, "gosling");
    }

    #[test]
    fn test_find_binary_in_package_subdir() {
        let tmp = tempdir().unwrap();
        let pkg = tmp.path().join("gosling-package");
    ...
    fn test_replace_binary_basic() {
        let tmp = tempdir().unwrap();
        let new_bin = tmp.path().join("new_gosling");
        let current = tmp.path().join("current_gosling");
    ...
    fn test_replace_binary_windows_rename_away() {
        let tmp = tempdir().unwrap();
        let current = tmp.path().join("gosling.exe");
        let new_bin = tmp.path().join("new_gosling.exe");
    ...
    fn test_extract_zip_with_package_dir() {
        ...
        // Create a zip in memory with gosling-package/ structure
        let mut buf = Vec::new();
        {
            ...
            writer.add_directory("gosling-package/", options).unwrap();
            writer
                .start_file("gosling-package/gosling.exe", options)
                .unwrap();
            writer.write_all(b"fake gosling binary").unwrap();
            writer
                .start_file("gosling-package/libtest.dll", options)
                .unwrap();
        ...
        let binary = find_binary(tmp.path(), "gosling.exe");
        assert!(binary.is_some());

        let content = fs::read_to_string(binary.unwrap()).unwrap();
        assert_eq!(content, "fake gosling binary");

        // DLL should be in gosling-package too
        assert!(tmp.path().join("gosling-package/libtest.dll").exists());
    }
    ...
    fn test_validate_entry_path_accepts_safe_paths() {
        assert!(validate_entry_path(Path::new("gosling")).is_ok());
        assert!(validate_entry_path(Path::new("gosling-package/gosling")).is_ok());
    ...
    fn test_extract_tar_bz2_safe_archive() {
        ...
        let mut builder_buf = Vec::new();
        {
            ...
            let data = b"gosling binary content";
            let mut header = tar::Header::new_gnu();
            header.set_size(data.len() as u64);
            header.set_mode(0o755);
            header.set_cksum();
            builder
                .append_data(&mut header, "gosling-package/gosling", &data[..])
                .unwrap();
        ...
        let extracted = tmp.path().join("gosling-package/gosling");
        assert!(extracted.exists());
        assert_eq!(
            fs::read_to_string(extracted).unwrap(),
            "gosling binary content"
        );
    }
