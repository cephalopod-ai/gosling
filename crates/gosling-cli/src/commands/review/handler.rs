...
pub async fn handle_review(opts: ReviewOptions) -> Result<()> {
    ...
    if diff.trim().is_empty() {
        eprintln!("gosling review: no changes to review");
        return Ok(());
    }
    ...
    if !use_orchestrator {
        ...
        if opts.checks_only {
            ...
            if !opts.quiet {
                ...
                eprintln!(
                    "gosling review: emitted {total_emitted} finding(s) from {} check(s) ({suppressed} hidden below severity={:?})",
                    discovered.checks.len(),
                    min_sev
                );
            }
    ...
    if !opts.quiet {
        ...
        if suppressed == 0 {
            eprintln!(
                "gosling review: orchestrator emitted {total_emitted} finding(s) from {} check(s) (main: {main_pass_label}, {} finding(s))",
                discovered.checks.len(),
                main_findings.len()
            );
        } else {
            eprintln!(
                "gosling review: orchestrator emitted {total_emitted} finding(s) from {} check(s) (main: {main_pass_label}, {} finding(s); {suppressed} hidden below severity={:?})",
                discovered.checks.len(),
                main_findings.len(),
                min_sev
            );
...
fn print_discovered_summary(d: &DiscoveredReview) {
    if d.checks.is_empty() {
        eprintln!("gosling review: no checks or REVIEW.md rules discovered");
        return;
    }
    eprintln!("gosling review: discovered {} check(s):", d.checks.len());
    for c in &d.checks {
...
/// `discovery_root` so [`gosling::checks::discover`] doesn't double-
