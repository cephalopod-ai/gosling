//! Deterministic, Rust-driven orchestration for `gosling review`.
//!
...
//!
//! - One subprocess per check (`gosling run -q -t <prompt>`)
...
/// Run all discovered checks concurrently as `gosling run` subprocesses.
///
...
pub async fn run_checks_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, check, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("gosling review: check task panicked: {e}");
                continue;
            }
        };
        ...
        match result {
            Ok(findings) => {
                if !quiet {
                    eprintln!(
                        "gosling review: check '{}' completed: {} finding(s)",
                        check.name,
                        findings.len()
                    );
                }
...
///    asks for `gosling-claude-4-sonnet` would 404 against Google's API),
...
/// Generic `gosling run` subprocess that hands a prompt to the model
...
pub async fn run_main_pass_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, path, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("gosling review: main-pass task panicked: {e}");
                continue;
            }
        };
        match result {
            Ok(raw) => {
                ...
                if !quiet {
                    eprintln!(
                        "gosling review: main pass on '{}' completed: {} finding(s)",
                        path,
                        findings.len()
                    );
                }
...
/// cap enforced by gosling. Without this, models routinely burn turns on
...
fn build_subprocess_turn_budget_section(max_turns: usize) -> String {
    format!(
        "## Turn budget\n\n\
         ...
         gosling enforces this via `--max-turns`; when you exhaust it, the session stops and \
...
mod tests {
    ...
    fn resolve_check_model_cli_provider_wins_over_per_check_model() {
        let mut c = ck("perf");
        c.model = Some("gosling-claude-4-sonnet".into()); // wrong provider
        let opts = ReviewOptions {
    ...
    fn resolve_check_model_cli_provider_alone_drops_per_check_model() {
        ...
        let mut c = ck("perf");
        c.model = Some("gosling-claude-4-sonnet".into());
        let opts = ReviewOptions {
            provider: Some("google".into()),
            default_model: None,
            ..ReviewOptions::default()
