...
use console::style;
use gosling::agents::extension::{ToolInfo, PLATFORM_EXTENSIONS};
use gosling::agents::extension_manager::get_parameter_names;
use gosling::agents::Agent;
use gosling::agents::{extension::Envs, ExtensionConfig};
use gosling::config::declarative_providers::{
...
async fn handle_first_time_setup(config: &Config) -> anyhow::Result<()> {
    ...
    println!();
    cliclack::intro(style(" gosling-configure ").on_cyan().black())?;
...
async fn handle_existing_config() -> anyhow::Result<()> {
    ...
    let action = cliclack::select("What would you like to configure?")
        ...
        .item(
            "settings",
            "gosling settings",
            "Set the gosling mode, Tool Output, Tool Permissions, Experiment and more",
        )
        .interact()?;
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let toolshim_enabled = std::env::var("GOSLING_TOOLSHIM")
...
/// Configure extensions that can be used with gosling
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&gosling::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
fn configure_stdio_extension() -> anyhow::Result<()> {
    ...
    let mut parts = gosling::utils::split_command_args(&command_str)?;
...
pub fn configure_extensions_dialog() -> anyhow::Result<()> {
    let extension_type = cliclack::select("What type of extension would you like to add?")
        .item(
            "built-in",
            "Built-in Extension",
            "Use an extension that comes with gosling",
        )
        .item(
            "stdio",
...
pub fn configure_gosling_mode_dialog() -> anyhow::Result<()> {
    let config = Config::global();

    if std::env::var("GOSLING_MODE").is_ok() {
        let _ = cliclack::log::info(
            "Notice: GOSLING_MODE environment variable is set and will override the configuration here.",
        );
    }

    let mode = cliclack::select("Which gosling mode would you like to configure?")
        .item(
            GoslingMode::Auto,
            "Auto Mode",
    ...
    let msg = match mode {
        GoslingMode::Auto => "Set to Auto Mode - full file modification enabled",
        GoslingMode::Approve => {
            "Set to Approve Mode - all tools and modifications require approval"
        }
        GoslingMode::SmartApprove => "Set to Smart Approve Mode - modifications require approval",
...
pub fn configure_tool_output_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("GOSLING_CLI_MIN_PRIORITY").is_ok() {
        let _ = cliclack::log::info(
            "Notice: GOSLING_CLI_MIN_PRIORITY environment variable is set and will override the configuration here.",
        );
    }
    ...
    match tool_log_level {
        "high" => {
            config.set_param("GOSLING_CLI_MIN_PRIORITY", 0.8)?;
            cliclack::outro("Showing tool output of high importance only.")?;
        }
        "medium" => {
            config.set_param("GOSLING_CLI_MIN_PRIORITY", 0.2)?;
            cliclack::outro("Showing tool output of medium importance.")?;
        }
        "all" => {
            config.set_param("GOSLING_CLI_MIN_PRIORITY", 0.0)?;
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("GOSLING_DISABLE_KEYRING").is_ok() {
        let _ = cliclack::log::info(
            "Notice: GOSLING_DISABLE_KEYRING environment variable is set and will override the configuration here.",
        );
    }

    let currently_disabled = config
        .get_param::<String>("GOSLING_DISABLE_KEYRING")
        .is_ok();
    ...
    match storage_option {
        "keyring" => {
            ...
            config.set_param("GOSLING_DISABLE_KEYRING", Value::String("".to_string()))?;
            cliclack::outro("Secret storage set to system keyring (secure)")?;
            let _ = cliclack::log::info(
                "You may need to restart gosling for this change to take effect",
            );
        }
        "file" => {
            // Set the disable flag to use file storage
            config.set_param("GOSLING_DISABLE_KEYRING", Value::String("true".to_string()))?;
            cliclack::outro(format!(
...
/// Configure experiment features that can be used with gosling
...
pub fn configure_max_turns_dialog() -> anyhow::Result<()> {
    ...
    let current_max_turns: u32 = config.get_param("GOSLING_MAX_TURNS").unwrap_or(1000);
    ...
    config.set_param("GOSLING_MAX_TURNS", max_turns)?;

    cliclack::outro(format!(
        "Set maximum turns to {} - gosling will ask for input after {} consecutive actions",
        max_turns, max_turns
    ))?;
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use gosling::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    ...
    let configured_model: String = config.get_gosling_model()?;
    let model_config =
        match gosling::model_config::model_config_from_user_config("openrouter", &configured_model)
        {
            Ok(config) => config,
            Err(e) => {
    ...
    match create("openrouter", Vec::new()).await {
        Ok(provider) => {
            let test_result = provider
                .complete(
                    &model_config,
                    "You are gosling, an AI assistant.",
            ...
            match test_result {
                Ok(_) => {
                    ...
                    if !has_developer {
                        set_extension(ExtensionEntry {
                            ...
                            config: ExtensionConfig::Platform {
                                ...
                                display_name: Some(
                                    gosling::config::DEFAULT_DISPLAY_NAME.to_string(),
                                ),
                                bundled: Some(true),
                                available_tools: Vec::new(),
                            },
                    ...
                    cliclack::outro("OpenRouter setup complete! You can now use gosling.")?;
                }
                Err(e) => {
...
pub async fn handle_tetrate_auth() -> anyhow::Result<()> {
    ...
    let configured_model: String = config.get_gosling_model()?;
    if let Err(e) =
        gosling::model_config::model_config_from_user_config("tetrate", &configured_model)
    {
        eprintln!("⚠️  Invalid model configuration: {}", e);
        eprintln!("Your settings have been saved. Please check your model configuration.");
        return Ok(());
...
fn add_provider() -> anyhow::Result<()> {
    ...
    if !provider_config.models.is_empty() {
        ...
        if let Ok(model) = cliclack::select("Which model should be the default?")
            ...
            .interact()
        {
            config.set_gosling_provider(&provider_config.name)?;
            config.set_gosling_model(model)?;
        }
    }
...
async fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = gosling::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        gosling::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
    ...
    if let Err(e) = gosling::providers::cleanup_provider(selected_id).await {
