//! Generate manpages for the gosling CLI.
//!
...
//! offline documentation accessible via the `man` command (e.g., `man gosling`).
//!
//! When gosling is packaged for Linux distributions (deb, rpm, etc.), the generated
...
//!
//! Output: target/man/gosling.1, target/man/gosling-session.1, etc.
...
fn generate_see_also(
    ...
    // Always reference the main gosling command if we're not it
    if current_name != "gosling" {
        references.push("gosling".to_string());
    }

    // Reference parent command if exists and not already added
    if let Some(parent) = parent_name {
        if parent != "gosling" && !references.contains(&parent.to_string()) {
    ...
    if current_name == "gosling" {
        ...
        for subcmd in cmd.get_subcommands() {
            ...
            if subcmd_name != "help" && !subcmd.is_hide_set() {
                let full_name = format!("gosling-{}", subcmd_name);
