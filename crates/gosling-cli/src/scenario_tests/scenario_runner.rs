...
use anyhow::Result;
use gosling::agents::{Agent, AgentConfig, GoslingPlatform};
use gosling::config::permission::PermissionManager;
use gosling::config::GoslingMode;
use gosling::providers::{create, testprovider::TestProvider};
...
{
    if let Ok(only_provider) = std::env::var("GOSLING_TEST_PROVIDER") {
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    gosling::agents::moim::SKIP.with(|f| f.set(true));
    ...
    let scenario_model_config =
        gosling::model_config::model_config_from_user_config(&factory_name, config.model_name)?;
    agent
        .update_provider(
            provider_arc as Arc<dyn gosling::providers::base::Provider>,
