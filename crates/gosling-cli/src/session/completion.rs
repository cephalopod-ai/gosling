...
/// Completer for gosling CLI commands
pub struct GoslingCompleter {
    pub completion_cache: Arc<std::sync::RwLock<CompletionCache>>,
    filename_completer: FilenameCompleter,
}

impl GoslingCompleter {
    /// Create a new GoslingCompleter with a reference to the Session's completion cache
    ...
    fn complete_mode_flags(&self, line: &str) -> Result<(usize, Vec<Pair>)> {
        let modes = GoslingMode::VARIANTS;
...
impl Completer for GoslingCompleter {
    type Candidate = Pair;
...
impl Helper for GoslingCompleter {}

// Implement required traits with default implementations
impl Hinter for GoslingCompleter {
    type Hint = String;
    ...
    fn hint(&self, line: &str, _pos: usize, _ctx: &Context<'_>) -> Option<Self::Hint> {
        ...
        match cache.hint_status {
            HintStatus::Interrupted => {
                Some("Interrupted, what should gosling work on instead?".to_string())
            }
...
impl Highlighter for GoslingCompleter {
...
impl Validator for GoslingCompleter {
    fn validate(
        &self,
...
mod tests {
    ...
    fn test_complete_slash_commands() {
        ...
        let completer = GoslingCompleter::new(cache);
