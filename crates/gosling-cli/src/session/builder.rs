...
/// Configuration for building a new Gosling session
///
...
async fn load_extensions(
    ...
    for (id, err) in failed {
        ...
        eprintln!(
            "{}",
            style(format!(
                "  Hint: once the session starts, ask gosling to help debug the '{}' extension",
                label
            ))
            .dim()
        );
    ...
    fn test_session_builder_config_creation() {
        let config = SessionBuilderConfig {
            ...
            streamable_http_extensions: vec![StreamableHttpOptions {
                ...
                timeout: gosling::config::DEFAULT_EXTENSION_TIMEOUT,
            }],
