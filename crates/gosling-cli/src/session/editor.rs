...
/// Checks GOSLING_PROMPT_EDITOR, then $VISUAL, then $EDITOR.
pub fn resolve_editor_command() -> Option<String> {
    let config = Config::global();
    let config_editor = config.get_gosling_prompt_editor().ok().flatten();
...
fn build_template(messages: &[&str], prefill: Option<&str>) -> String {
    let mut content = String::from("# Gosling Prompt Editor\n\n");
...
pub fn get_editor_input(
    ...
    // Read back by path rather than through the original handle: editors that save by
    // writing a replacement file and renaming it over the target leave the first inode stale.
...
mod tests {
    ...
    fn test_extract_user_input_with_editor_output() {
        let content = r#"# Gosling Prompt Editor
...
"#;
    ...
    fn test_create_temp_file_with_messages() {
        ...
        assert!(path.exists());
        assert!(path.to_str().unwrap().contains("gosling_prompt_"));
        assert!(path.to_str().unwrap().ends_with(".md"));

        let content = fs::read_to_string(path).unwrap();
        assert!(content.contains("# Gosling Prompt Editor"));
    ...
    fn test_create_temp_file_with_prefix_suffix() {
        let temp_file = Builder::new()
            .prefix("gosling_test_")
            .suffix(".md")
            .tempfile()
            .unwrap();

        let name = temp_file.path().file_name().unwrap().to_str().unwrap();
        assert!(name.starts_with("gosling_test_"));
...
"#;
    ...
    fn test_build_template_no_prefill_no_messages() {
        ...
        assert_eq!(content, "# Gosling Prompt Editor\n\n# Your prompt:\n\n");
    }
