...
use gosling::subprocess::SubprocessExt;
    ...
    fn as_str(&self) -> String {
        match self {
            Theme::Light => Config::global()
                .get_param::<String>("GOSLING_CLI_LIGHT_THEME")
                .unwrap_or(DEFAULT_CLI_LIGHT_THEME.to_string()),
            Theme::Dark => Config::global()
                .get_param::<String>("GOSLING_CLI_DARK_THEME")
...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        std::env::var("GOSLING_CLI_THEME").ok()
            .map(|val| Theme::from_config_str(&val))
            .unwrap_or_else(||
                Config::global().get_param::<String>("GOSLING_CLI_THEME").ok()
                    .map(|val| Theme::from_config_str(&val))
                    .unwrap_or(Theme::Ansi)
            )
    );
    static SHOW_FULL_TOOL_OUTPUT: RefCell<bool> = RefCell::new(
        Config::global().get_param::<bool>("GOSLING_SHOW_FULL_OUTPUT").unwrap_or(false)
    );
}
...
pub fn set_theme(theme: Theme) {
    ...
    config
        .set_param("GOSLING_CLI_THEME", theme.as_config_string())
    ...
    if let Err(e) = config.set_param("GOSLING_CLI_THEME", theme_str) {
...
pub fn run_status_hook(status: &str) {
    ...
    if let Ok(hook) = Config::global().get_param::<String>("GOSLING_STATUS_HOOK") {
...
fn should_show_thinking() -> bool {
    Config::global()
        .get_param::<bool>("GOSLING_CLI_SHOW_THINKING")
        .unwrap_or(false)
...
pub fn display_session_info(
    ...
    // ASCII art gosling with session info on the right
    println!();
    println!(
        "  {}  {} {} {} {} {}",
        style("  __( O)>").white(),
