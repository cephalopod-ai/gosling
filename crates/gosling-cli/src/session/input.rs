use super::completion::GoslingCompleter;
...
pub fn get_newline_key() -> char {
    Config::global()
        .get_param::<String>("GOSLING_CLI_NEWLINE_KEY")
        .ok()
...
///
/// When `gosling_prompt_editor` is configured, defaults to `true` (backward compat).
/// Users can override by explicitly setting `gosling_prompt_editor_always` to `false`.
...
pub fn get_input(
    ...
    let prompt_editor = config.get_gosling_prompt_editor().ok().flatten();
    let editor_always_override = config.get_gosling_prompt_editor_always().ok().flatten();
    let editor_always = should_use_editor_always(prompt_editor.as_deref(), editor_always_override);

    if editor_always {
        if let Ok(Some(editor_cmd)) = config.get_gosling_prompt_editor() {
...
fn print_help() {
    ...
    let modes = GoslingMode::VARIANTS.join(", ");
    println!(
        "Available commands:
...
Ctrl+{newline_key} - Add a newline (configurable via GOSLING_CLI_NEWLINE_KEY)
