...
use gosling::config::declarative_providers::load_provider;
use gosling::config::paths::Paths;
use gosling::config::{Config, ConfigError, DeclarativeProviderConfig};
use gosling_test_support::EnforceSessionId;
...
fn acp_catalog_and_custom_provider_methods_use_core_provider_store() {
    ...
    run_test(async move {
        ...
        let template = send_custom(
            ...
            serde_json::json!({ "providerId": "deepseek" }),
        )
        .await
        ...
        assert!(
            ...
            "custom provider should be saved in Gosling's declarative provider store"
        );
        ...
        assert_eq!(
            Config::global()
                ...
                .unwrap(),
            "created-custom-key",
            "custom provider create should write through Gosling's config store"
        );
        assert!(
        ...
        assert_eq!(
            Config::global()
                ...
                .unwrap(),
            "updated-custom-key",
            "custom provider update should write through Gosling's config store"
        );
        ...
        for valid_id in ["custom_openai", "openai-compat", "a1"] {
            assert!(
                gosling::config::declarative_providers::validate_provider_id(valid_id).is_ok(),
