...
use gosling::agents::extension_manager::{ExtensionManager, ExtensionManagerCapabilities};
...
impl gosling::providers::base::ProviderDescriptor for MockProvider {
...
async fn test_replayed_session(
    ...
    let _env = env_lock::lock_env([
        ("GOSLING_MCP_CLIENT_VERSION", Some("0.0.0")),
        ("GOSLING_PROVIDER", Some("openai")),
        ("GOSLING_MODEL", Some("gpt-4o")),
    ]);

    // Setup test file for developer extension tests
    let test_file_path = "/tmp/gosling_test/gosling.txt";
    if let Some(parent) = std::path::Path::new(test_file_path).parent() {
        fs::create_dir_all(parent).ok();
    }
    fs::write(test_file_path, "# gosling\n").ok();
    ...
    let mode = if env::var("GOSLING_RECORD_MCP").is_ok() {
        TestMode::Record
    } else {
