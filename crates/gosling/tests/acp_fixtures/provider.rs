...
use gosling::acp::{AcpProvider, AcpProviderConfig};
...
use gosling_test_support::{ExpectedSessionId, IgnoreSessionId, TEST_MODEL};
...
impl AcpProviderSession {
    ...
    async fn send_message(
        ...
        let mut stream = gosling::session_context::with_session_id(
...
impl Connection for AcpProviderConnection {
    ...
    async fn new(config: TestConnectionConfig, openai: OpenAiFixture) -> Self {
        ...
        let gosling_mode = config.gosling_mode;
        ...
        let provider_config = AcpProviderConfig {
            ...
            mode_mapping: GoslingMode::VARIANTS
                .iter()
                .map(|v| {
                    let mode = GoslingMode::from_str(v).unwrap();
                    (mode, mode.to_string())
                })
                .collect(),
    ...
    async fn new_session(&mut self) -> anyhow::Result<SessionData<AcpProviderSession>> {
        ...
        let gosling_id = format!("test-session-{}", self.session_counter);
        ...
        let session = AcpProviderSession {
            ...
            session_id: agent_client_protocol::schema::v1::SessionId::new(gosling_id),
