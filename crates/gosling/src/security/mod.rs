...
impl SecurityManager {
    ...
    pub async fn analyze_tool_requests(
        ...
        if !self.is_prompt_injection_detection_enabled() {
            tracing::debug!(
                monotonic_counter.gosling.prompt_injection_scanner_disabled = 1,
        ...
        let scanner = self.scanner.get_or_init(|| {
            ...
            tracing::info!(
                monotonic_counter.gosling.security_command_classifier_enabled = if command_classifier_enabled { 1 } else { 0 },
                monotonic_counter.gosling.security_prompt_classifier_enabled = if prompt_classifier_enabled { 1 } else { 0 },
            ...
            let scanner = if ml_enabled {
                match PromptInjectionScanner::with_ml_detection() {
                    Ok(s) => {
                        tracing::info!(
                            monotonic_counter.gosling.prompt_injection_scanner_enabled = 1,
