...
//! ```
//!
//! Gosling currently supports `type: "command"` actions. Unknown event names and
...
mod tests {
    ...
    async fn command_hooks_repair_path_when_enabled() {
        ...
        let _guard = env_lock::lock_env([
            ("GOSLING_SHELL", Some(fake_shell.as_str())),
            ("FAKE_LOGIN_PATH", Some(fake_login_path.as_str())),
            (
                "PATH",
                Some(
                    "/Applications/Gosling.app/Contents/Resources/bin:/usr/bin:/bin:/usr/sbin:/sbin",
                ),
            ),
        ]);
