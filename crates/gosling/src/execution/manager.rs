use crate::agents::mcp_client::GoslingMcpHostInfo;
use crate::agents::{Agent, AgentConfig, ExtensionLoadResult, GoslingPlatform};
...
impl AgentManager {
    ...
    async fn create_agent_locked(
        ...
        let mut mode = self.agent_config.gosling_mode;
        if let Ok(session) = self
            ...
            .await
        {
            mode = session.gosling_mode;
            info!(gosling_mode = %mode, session_id = %session_id, "Session loaded");
        }

        let mut config = self.agent_config.clone();
        config.gosling_mode = mode;
        ...
        if agent.provider().await.is_err() {
            if let Some(provider) = &*self.default_provider.read().await {
                ...
                let model_config = config
                    ...
                    .ok()
                    .zip(config.get_gosling_model().ok())
                    ...
                    .unwrap_or_else(|| gosling_providers::model::ModelConfig::new("unknown"));
                agent
...
mod tests {
    ...
    #[test_case(GoslingMode::Chat ; "chat")]
    #[test_case(GoslingMode::SmartApprove ; "smart_approve")]
    #[tokio::test]
    async fn test_agent_inherits_session_mode(mode: GoslingMode) {
        let temp_dir = TempDir::new().unwrap();
        let manager = create_test_manager(&temp_dir).await;
        ...
        assert_eq!(agent.gosling_mode().await, mode);
    }
    ...
    async fn test_session_mode_isolation() {
        ...
        assert_eq!(a1.gosling_mode().await, GoslingMode::Approve);
        assert_eq!(a2.gosling_mode().await, GoslingMode::Auto);
    }
}
