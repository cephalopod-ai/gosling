...
pub struct CodexProvider {
    ...
    #[serde(skip)]
    mode_by_session: tokio::sync::RwLock<HashMap<String, GoslingMode>>,
}
...
impl CodexProvider {
    ...
    async fn execute_command(
        ...
        if std::env::var("GOSLING_CODEX_DEBUG").is_ok() {
        ...
        // This allows users to set GOSLING_PROVIDER=codex without needing to specify a model
        ...
        Self::apply_permission_flags(&mut cmd, gosling_mode)?;
...
impl Provider for CodexProvider {
    ...
    async fn stream(
        ...
        let lines = self
            .execute_command(model_config, system, messages, tools, gosling_mode)
            .await?;
...
mod tests {
    ...
    #[test_case(GoslingMode::SmartApprove, &["--full-auto"] ; "smart_approve_full_auto")]
    #[test_case(GoslingMode::Approve, &[] as &[&str] ; "approve_no_flags")]
    #[test_case(GoslingMode::Chat, &["--sandbox", "read-only"] ; "chat_read_only")]
    fn test_apply_permission_flags(mode: GoslingMode, expected: &[&str]) {
