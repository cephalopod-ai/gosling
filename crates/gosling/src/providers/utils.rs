...
use fs_err::File;
use gosling_providers::errors::{GoogleErrorCode, ProviderError};
use gosling_providers::request_log::{install_logger, RequestLogHandle, RequestLogger};
            ...
            if let Some(payload) = &payload {
                if let Some(error) = payload.get("error") {
                    ...
                    if error_status == "INVALID_ARGUMENT"
                        && gosling_providers::http_status::is_context_length_exceeded_message(&error_msg)
                    {
