...
impl gosling_providers::base::ProviderDescriptor for CodexAcpProvider {
    fn metadata() -> ProviderMetadata {
        ProviderMetadata::new(
            CODEX_ACP_PROVIDER_NAME,
            "Codex CLI",
            "Use gosling with your ChatGPT Plus/Pro subscription via the codex-acp adapter.",
            ACP_CURRENT_MODEL,
            vec![],
            CODEX_ACP_DOC_URL,
            vec![],
...
// So Approve uses read-only sandbox to force write approvals through gosling.
fn map_gosling_mode(gosling_mode: GoslingMode) -> (&'static str, &'static str) {
    match gosling_mode {
        GoslingMode::Auto => ("never", "danger-full-access"),
        GoslingMode::SmartApprove => ("on-request", "workspace-write"),
        GoslingMode::Approve => ("on-request", "read-only"),
        GoslingMode::Chat => ("never", "read-only"),
    }
}
...
mod tests {
    ...
    #[test_case(GoslingMode::Auto, "never", "danger-full-access")]
    #[test_case(GoslingMode::SmartApprove, "on-request", "workspace-write")]
    #[test_case(GoslingMode::Approve, "on-request", "read-only")]
    #[test_case(GoslingMode::Chat, "never", "read-only")]
    fn test_map_gosling_mode(mode: GoslingMode, expected_approval: &str, expected_sandbox: &str) {
