...
use gosling_providers::images::{convert_image, detect_image_path, load_image_file, ImageFormat};
use rmcp::model::{
        ...
        if let Some(tool_calls_array) = tool_calls.as_array() {
            for tool_call in tool_calls_array {
                ...
                if !is_valid_function_name(&function_name) {
                    ...
                    match gosling_providers::json::parse_tool_arguments(&arguments_str) {
                        ...
                        None => {
                            let message_text =
                                gosling_providers::json::truncation_error_message(&arguments_str)
                                    .unwrap_or_else(|| {
                                        format!(
...
pub fn create_request_for_provider(
    ...
    if model_config.model_name.starts_with("o1-mini") {
        return Err(anyhow!(
            "o1-mini model is not currently supported since gosling uses tool calling and o1-mini does not support it. Please use o1 or o3 models instead."
        ));
    }
...
mod tests {
    ...
    fn test_create_request_adaptive_thinking_for_new_anthropic_models() -> anyhow::Result<()> {
        let _guard = env_lock::lock_env([("GOSLING_THINKING_EFFORT", None::<&str>)]);
    ...
    fn test_is_claude_model() {
        ...
        assert!(is_claude_model("gosling-claude-sonnet"));
