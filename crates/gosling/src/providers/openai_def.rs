...
use gosling_providers::base::ProviderDescriptor;
...
pub async fn from_env(
    tls_config: Option<gosling_providers::api_client::TlsConfig>,
    ...
    //      honoured so that `OPENAI_HOST=… gosling` keeps working)
    //   2. OPENAI_BASE_URL (env or config) — ecosystem-standard
    //   3. OPENAI_HOST from config file — persisted by `gosling configure`
            ...
            match e {
                ConfigError::NotFound(_) => {
                    if config.requires_auth {
                        anyhow::bail!(
                            "Required API key {} is not set. Configure it via `gosling configure` or set the {} environment variable.",
                            config.api_key_env,
                            config.api_key_env
                        );
                    }
...
pub fn from_custom_config(
    ...
    gosling_providers::openai::from_declarative_config(
        config,
        tls_config,
