...
impl gosling_providers::base::ProviderDescriptor for KimiCodeProvider {
    fn metadata() -> ProviderMetadata {
        ProviderMetadata::new(
            ...
            KIMI_CODE_DOC_URL,
            // Marker key — the actual token lives in ~/.config/gosling/kimicode/token.json.
        ...
        .with_setup_steps(vec![
            "Run `gosling configure` and select 'Kimi Code'",
            "A browser window will open — log in to kimi.com and enter the displayed code",
            "Once authorized, Gosling will save your token automatically",
        ])
    }
}
        ...
        KimiCodeProvider {
            ...
            token_cache: TokenCache {
                path: std::env::temp_dir()
                    .join(format!("gosling-kimicode-test-{}.json", Uuid::new_v4())),
            },
            cached_token: tokio::sync::Mutex::new(None),
