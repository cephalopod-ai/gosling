...
pub struct LoggingConfig<'a> {
    ...
    /// Additional `EnvFilter` directives beyond the defaults (e.g. "gosling_server=info").
...
fn build_env_filter(extra_directives: &[&str]) -> EnvFilter {
    ...
    let mut filter = EnvFilter::new("")
        ...
        .add_directive("gosling=info".parse().unwrap())
...
/// Set up file-based (and optionally console) tracing for a gosling component.
///
/// This is the shared implementation used by both `gosling-cli` and `gosling-server`.
