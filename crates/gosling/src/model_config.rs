...
pub fn model_config_from_user_config_with_session_settings(
    ...
    let model = materialize_model_config_inner(model, provider_name, false)?
        ...
        .with_default_thinking_effort(config.get_gosling_thinking_effort());
...
fn materialize_model_config_inner(
    ...
    if model.temperature.is_none() {
        model = model.with_temperature(get_gosling_temperature(config)?);
    }

    if model.toolshim && model.toolshim_model.is_none() {
        model = model.with_toolshim_model(get_gosling_toolshim_model(config)?);
    }

    model = model
        .with_default_context_limit(config.get_gosling_context_limit()?)
        .with_default_max_tokens(config.get_gosling_max_tokens()?);

    if matches!(provider_name, "chatgpt_codex" | "codex") {
        let context_limit =
    ...
    if include_default_thinking_effort {
        model = model.with_default_thinking_effort(config.get_gosling_thinking_effort());
    }

    if provider_name == gosling_providers::openai::OPEN_AI_PROVIDER_NAME {
...
fn configured_fast_model_name() -> Option<String> {
    Config::global()
        .get_param::<String>("GOSLING_FAST_MODEL")
        .ok()
...
///   1. `GOSLING_FAST_MODEL` (user override)
...
fn base_model_config_from_user_config(model_name: &str) -> Result<ModelConfig> {
    ...
    let mut model = ModelConfig {
        ...
        context_limit: None,
        temperature: get_gosling_temperature(config)?,
        max_tokens: None,
        toolshim: get_gosling_toolshim(config)?.unwrap_or(false),
        toolshim_model: get_gosling_toolshim_model(config)?,
        request_params: None,
        reasoning: None,
    };
...
fn get_gosling_temperature(config: &Config) -> Result<Option<f32>> {
    match config.get_param::<f32>("GOSLING_TEMPERATURE") {
        Ok(temp) if temp < 0.0 => Err(anyhow!(
            "Value for 'GOSLING_TEMPERATURE' is out of valid range: {temp}"
        )),
...
fn get_gosling_toolshim(config: &Config) -> Result<Option<bool>> {
    match config.get_param::<serde_yaml::Value>("GOSLING_TOOLSHIM") {
        Ok(value) => parse_yaml_bool_config("GOSLING_TOOLSHIM", value).map(Some),
        Err(ConfigError::NotFound(_)) => Ok(None),
        Err(e) => Err(e.into()),
    }
}
...
pub fn global_toolshim() -> bool {
    get_gosling_toolshim(Config::global())
        .ok()
        .flatten()
        .unwrap_or(false)
}

fn get_gosling_toolshim_model(config: &Config) -> Result<Option<String>> {
    match config.get_param::<String>("GOSLING_TOOLSHIM_OLLAMA_MODEL") {
        Ok(value) if value.trim().is_empty() => Err(anyhow!(
            "Invalid value for 'GOSLING_TOOLSHIM_OLLAMA_MODEL': '{value}' - cannot be empty if set"
        )),
