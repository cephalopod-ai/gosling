use super::{build_session_info, meta_string, GoslingAcpAgent, ResultExt};
...
struct SessionListCursorToken {
    ...
    // Gosling stores timestamps with second precision in common write paths, so the
...
fn include_last_message_snippet_from_meta(
    ...
    let Some(value) = meta.and_then(|meta| meta.get("gosling")) else {
        return Ok(false);
    };
    if value.is_null() {
        return Ok(false);
    ...
    let Some(gosling_meta) = value.as_object() else {
        return Err(
            agent_client_protocol::Error::invalid_params().data("gosling must be an object")
        );
    };
    let Some(value) = gosling_meta.get("includeLastMessageSnippet") else {
        return Ok(false);
    };
    if value.is_null() {
        return Ok(false);
    ...
    value.as_bool().ok_or_else(|| {
        agent_client_protocol::Error::invalid_params()
            .data("gosling.includeLastMessageSnippet must be a boolean")
    })
}
...
fn archive_state_from_meta(
    ...
    let Some(value) = gosling_meta.get("archiveState") else {
        return Ok(SessionArchiveState::Active);
    };
    if value.is_null() {
        return Ok(SessionArchiveState::Active);
    ...
    match value.as_str() {
        ...
        _ => Err(agent_client_protocol::Error::invalid_params()
            .data("gosling.archiveState must be one of active, archived, or all")),
    }
}
