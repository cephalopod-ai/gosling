...
const GOSLING_CONFIG_PREFIX: &str = "gosling_config:";
...
impl GoslingAcpAgent {
    pub(super) async fn on_onboarding_import_scan(
        ...
        if source_filter.contains(&OnboardingImportSourceKind::GoslingConfig) {
            for path in gosling_config_candidate_paths(&self.config_dir) {
                if let Some(candidate) = scan_gosling_config_candidate(&path) {
...
fn apply_onboarding_import_candidates(
    ...
    for candidate_id in &req.candidate_ids {
        match parse_candidate_id(candidate_id) {
            Some((OnboardingImportSourceKind::GoslingConfig, path)) => {
                match apply_gosling_config_candidate(config, target_config_dir, &path) {
                    Ok(result) => {
                    ...
                    Err(error) => warnings.push(import_failure_warning(
                        OnboardingImportSourceKind::GoslingConfig,
                        &path,
                        &error,
                    )),
                }
...
fn import_failure_warning(
    ...
    let source_name = match source_kind {
        OnboardingImportSourceKind::GoslingConfig => "Gosling configuration",
...
fn scan_gosling_config_candidate(path: &Path) -> Option<OnboardingImportCandidate> {
    if !path.exists() {
        return None;
    }
    ...
    if mapping_contains_string(&mapping, "GOSLING_PROVIDER")
        || mapping_contains_string(&mapping, "GOSLING_MODEL")
    {
        counts.providers = 1;
    }
    ...
    if counts.sessions > 0 {
        warnings.push("Sessions are already shared through Gosling's data store.".to_string());
    }

    Some(OnboardingImportCandidate {
        id: candidate_id(GOSLING_CONFIG_PREFIX, path),
        source_kind: OnboardingImportSourceKind::GoslingConfig,
        display_name: "Existing Gosling configuration".to_string(),
...
fn parse_candidate_id(id: &str) -> Option<(OnboardingImportSourceKind, PathBuf)> {
    if let Some(path) = id.strip_prefix(GOSLING_CONFIG_PREFIX) {
        return Some((
            OnboardingImportSourceKind::GoslingConfig,
            PathBuf::from(path),
        ));
...
fn apply_gosling_config_candidate(
    ...
    let provider = yaml_string(&source, "GOSLING_PROVIDER");
    let model = yaml_string(&source, "GOSLING_MODEL");
    ...
    let extension_result = import_gosling_config_extensions(target_config, &source)?;
    ...
    result.warnings.push(
        "Session history already lives in the Gosling data store when available.".to_string(),
    );
    Ok(result)
}
...
fn import_gosling_config_extensions(
    target_config: &Config,
    source: &Mapping,
...
mod tests {
    ...
    fn apply_onboarding_imports_continues_after_candidate_failure() {
        ...
        let missing_gosling_config = source.path().join("missing-config.yaml");
        ...
        let req = OnboardingImportApplyRequest {
            candidate_ids: vec![
                candidate_id(GOSLING_CONFIG_PREFIX, &missing_gosling_config),
        ...
        assert!(response
            .warnings
            .iter()
            .any(|warning| warning.starts_with("Skipped Gosling configuration import at ")));
    ...
    #[test]
    fn apply_gosling_config_imports_defaults_extensions_and_skills() {
        let source = TempDir::new().unwrap();
        let target = TempDir::new().unwrap();
...
"#,
        ...
        let result =
            apply_gosling_config_candidate(&target_config, target.path(), &source_config).unwrap();

        assert_eq!(result.imported.providers, 1);
        assert_eq!(result.imported.extensions, 1);
        assert_eq!(result.imported.skills, 1);
        assert_eq!(target_config.get_gosling_provider().unwrap(), "openai");
        assert!(target.path().join("skills").join("reviewer").exists());
    }

    #[test]
    fn apply_gosling_config_model_only_skips_provider_activation() {
        let source = TempDir::new().unwrap();
        let target = TempDir::new().unwrap();
        let source_config = source.path().join(CONFIG_YAML_NAME);
        fs::write(&source_config, "GOSLING_MODEL: gpt-5.1\n").unwrap();
        ...
        assert!(target_config.get_gosling_provider().is_err());
    }
