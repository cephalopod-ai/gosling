...
This block is generated by gosling and contains current operational context such as:
- current time
- working directory
- compaction status
- turn budget
            ...
            for (label, block) in &cases {
                assert!(
                    ...
                    "is_turn_context_text rejected real compose_moim output ({label}); the \
                     producer in gosling and the detector in gosling-providers have drifted, so the \
