...
/// Resolve the preferred Unix shell for command execution, respecting GOSLING_SHELL.
///
...
/// the host filesystem. `GOSLING_SHELL` is passed through as-is.
///
#[cfg(windows)]
fn windows_shell() -> String {
    std::env::var("GOSLING_SHELL").unwrap_or_else(|_| "cmd".to_string())
}
...
fn unix_shell() -> String {
    if let Ok(shell) = std::env::var("GOSLING_SHELL") {
        return shell;
    }
...
///
/// When goslingd is launched from a desktop app (e.g. Electron), it may inherit
...
pub(crate) fn resolve_login_shell_path() -> Option<String> {
    ...
    // (TIOCSPGRP) cannot steal the terminal foreground from gosling, which
    // would cause gosling to receive SIGTTIN and be suspended on startup.
