...
use crate::session_context::{
    SESSION_ID_HEADER, TOOL_CALL_REQUEST_ID_HEADER, TOOL_OPERATION_ID_HEADER, WORKING_DIR_HEADER,
};
use rmcp::model::{
...
/// MCP client implementation for Gosling
use rmcp::{
    model::{
...
fn resolve_sampling_model_config() -> anyhow::Result<gosling_providers::model::ModelConfig> {
    ...
    let provider_name = config.get_gosling_provider()?;
    let model_name = config.get_gosling_model()?;
...
impl GoslingClient {
    ...
    fn resolved_client_info(&self) -> Implementation {
        ...
        let version = self
            ...
            .unwrap_or_else(|| {
                std::env::var("GOSLING_MCP_CLIENT_VERSION")
...
impl ClientHandler for GoslingClient {
    ...
    async fn create_message(
        ...
        let system_prompt = params
            .system_prompt
            .as_deref()
            .unwrap_or("You are a general-purpose AI agent called gosling");
...
#[derive(Debug, Clone)]
pub struct GoslingMcpClientCapabilities {
    pub mcpui: bool,
    ...
    async fn call_tool(
        ...
        let result = self
            .send_request_with_context(
                ...
                ctx.tool_operation_id.as_deref(),
                request,
                cancel_token,
            )
            .await;
...
fn inject_session_context_into_extensions(
    ...
    let tool_operation_id = tool_operation_id.filter(|id| !id.is_empty());
    ...
    meta_map.retain(|k, _| {
        !k.eq_ignore_ascii_case(SESSION_ID_HEADER)
            ...
            && !k.eq_ignore_ascii_case(TOOL_OPERATION_ID_HEADER)
    });
    ...
    if let Some(tool_operation_id) = tool_operation_id {
        meta_map.insert(
            TOOL_OPERATION_ID_HEADER.to_string(),
            Value::String(tool_operation_id.to_string()),
        );
    }
...
mod tests {
    ...
    fn new_client(platform: GoslingPlatform) -> GoslingClient {
        let capabilities = match platform {
            GoslingPlatform::GoslingDesktop => GoslingMcpClientCapabilities {
                mcpui: true,
                host_info: None,
            },
            GoslingPlatform::GoslingCli => GoslingMcpClientCapabilities {
                mcpui: false,
                host_info: None,
            },
        };
    ...
    fn test_request_injects_session(request_builder: fn(Extensions) -> ClientRequest) {
        ...
        extensions.insert(
            serde_json::from_value::<Meta>(json!({
                "Gosling-Session-Id": "old-session-id",
        ...
        let request =
            inject_session_context_into_request(request, Some(session_id), None, None, None);
    ...
    fn test_session_id_case_insensitive_replacement(
        ...
        let extensions =
            inject_session_context_into_extensions(extensions, session_id, None, None, None);
        let mcp_meta = extensions.get::<Meta>().unwrap();
    ...
    fn test_tool_call_request_id_injected_only_for_call_tool() {
        ...
        let tool_operation_id = "toolop-1";
        ...
        assert_eq!(
            call_meta.0.get(TOOL_OPERATION_ID_HEADER),
            Some(&Value::String(tool_operation_id.to_string()))
        );
    ...
    fn test_explicit_host_info_passes_through_client_identity() {
        let client = GoslingClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
            GoslingPlatform::GoslingDesktop.to_string(),
            GoslingMcpClientCapabilities {
                mcpui: true,
                host_info: Some(GoslingMcpHostInfo {
                    explicit_extensions: true,
                    extensions: ExtensionCapabilities::new(),
                    client_name: Some("gosling2".to_string()),
        ...
        let info = ClientHandler::get_info(&client);
        assert_eq!(info.client_info.name, "gosling2");
    ...
    fn test_explicit_host_extensions_override_platform_fallback() {
        let client = GoslingClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
            GoslingPlatform::GoslingCli.to_string(),
            GoslingMcpClientCapabilities {
                mcpui: false,
                host_info: Some(GoslingMcpHostInfo {
                    explicit_extensions: true,
