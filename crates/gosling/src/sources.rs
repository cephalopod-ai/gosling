...
fn list_agent_dirs(working_dir: Option<&Path>, additional_roots: &[SourceRoot]) -> Vec<SourceRoot> {
    ...
    if let Some(working_dir) = working_dir {
        ...
        dirs.push(SourceRoot {
            path: working_dir.join(".gosling").join("agents"),
            writable: true,
        });
        dirs.push(SourceRoot {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        dirs.push(SourceRoot {
            path: home.join(".gosling").join("agents"),
            writable: true,
        });
        dirs.push(SourceRoot {
...
mod tests {
    ...
    fn list_sources_lists_builtin_skills() {
        ...
        let builtin = listed
            .iter()
            .find(|source| source.name == "gosling-doc-guide")
        ...
        assert_eq!(builtin.path, "builtin://skills/gosling-doc-guide");
    ...
    fn filesystem_skill_suppresses_same_named_builtin() {
        ...
        assert!(!builtins
            .iter()
            .any(|source| source.name == "gosling-doc-guide"));
    ...
    #[test]
    fn project_sources_prefer_agents_directory_over_legacy_gosling() {
        let tmp = TempDir::new().unwrap();
        ...
        std::fs::write(
            ...
            build_skill_md("shared-skill", "legacy", "Gosling", &HashMap::new()),
        )
        .unwrap();
    ...
    fn update_rejects_path_traversal() {
        let tmp = TempDir::new().unwrap();
        let project = tmp.path();
        let escaped_dir = project.join(".gosling").join("escaped");
        ...
        let attempted_escape = project.join(".gosling").join("escaped");
        let err = update_source_with_roots(
            SourceType::Skill,
