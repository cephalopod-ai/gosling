...
//! (`GOSLING_CONTEXT_MANAGER`), gosling-2's fork-specific feature over base
//! gosling.
//!
...
async fn eval_one(
    ...
    match result {
        Ok(packet) => {
            let (_, final_messages) = goose::context_mgmt::resolve_provider_input(
                mode,
                &packet,
...
async fn eval_all_scenarios_context_manager() {
    ...
    let out_path = std::env::var("MEMORY_EVAL_OUTPUT")
        .unwrap_or_else(|_| "/tmp/gosling2_context_manager_eval_results.json".to_string());
