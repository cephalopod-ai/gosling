# Long-conversation memory evaluation: gosling-2 Context Manager vs. base gosling

This is a comparative evaluation of gosling-2's fork-specific "memory manager"
feature — internally named the **Context Manager**
(`GOSLING_CONTEXT_MANAGER=off|shadow|on`) — against base gosling's only
...
and its counterpart of the same name in the `gosling` repo (baseline). Both
...
outer LLM call (compaction's summarization step, which only base gosling's
mechanism needs) replaced by deterministic mock providers. gosling-2's
...
Where base gosling's recall depends on real summarizer quality (which cannot
...
### gosling-2 Context Manager (`on` mode — the packet actually used as provider input)
...
### gosling baseline (`compact_messages`, LLM-based, both repos share this code unchanged)
...
compacts the entire conversation in a single LLM call, unlike gosling-2's
...
| | Base gosling (`compact_messages`) | gosling-2 Context Manager (`on`) |
|---|---|---|
...
\* Base gosling does have a separate, pre-existing MCP "memory" extension
(`crates/goose-mcp/src/memory`, `remember_memory`/`retrieve_memories` tools) —
...
as base gosling's LLM-based compaction (content buried inside tool output is
...
capability base gosling has no equivalent of for automatic, cross-session
...
- `generic_worst_case` / `faithful_best_case` bound real LLM summarization
  quality; they don't reproduce it. Real-world recall for base gosling's
