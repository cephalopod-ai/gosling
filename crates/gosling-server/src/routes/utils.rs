...
use gosling::providers::base::{ConfigKey, ProviderMetadata, ProviderType};
...
pub fn check_provider_configured(metadata: &ProviderMetadata, provider_type: ProviderType) -> bool {
    ...
    if has_oauth_key {
        if let Some(entry) = gosling::config::get_provider_entry(config, &metadata.name) {
            if entry.configured {
                return true;
            }
        }
    ...
    if metadata.config_keys.is_empty() {
        ...
        if let Ok(current) = config.get_gosling_provider() {
