...
use gosling::config::declarative_providers::LoadedProvider;
use gosling::config::paths::Paths;
use gosling::config::ExtensionEntry;
use gosling::config::{Config, ConfigError};
use gosling::custom_requests::SourceType;
use gosling::providers::base::{ModelInfo, ProviderMetadata, ProviderType};
use gosling::providers::canonical::maybe_get_canonical_model;
use gosling::providers::catalog::{
    get_provider_template, get_providers_by_format, ProviderCatalogEntry, ProviderFormat,
    ProviderTemplate,
};
use gosling::providers::create_with_default_model;
use gosling::providers::providers as get_providers;
use gosling::{agents::execute_commands, agents::ExtensionConfig};
...
pub use gosling::providers::provider_secrets::{
...
pub async fn upsert_config(
    ...
    if query.key == "GOSLING_PROVIDER" {
        if let Some(name) = query.value.as_str() {
            let model = gosling::config::get_provider_entry(config, name)
                .map(|e| e.model)
                .or_else(|| config.get_gosling_model().ok())
                .unwrap_or_default();
            gosling::config::set_active_provider(config, name, &model)?;
            return Ok(Json(Value::String(format!("Upserted key {}", query.key))));
        }
    }
    if query.key == "GOSLING_MODEL" {
        if let Some(model) = query.value.as_str() {
            if let Ok(provider) = config.get_gosling_provider() {
                gosling::config::set_active_provider(config, &provider, model)?;
                return Ok(Json(Value::String(format!("Upserted key {}", query.key))));
            }
        }
    }
...
pub async fn remove_config(
    ...
    } else if query.key == "GOSLING_PROVIDER" || query.key == "active_provider" {
        config.delete("active_provider")?;
        config.delete("GOSLING_PROVIDER")?;
    } else if query.key == "GOSLING_MODEL" {
        if let Ok(provider) = config.get_gosling_provider() {
            gosling::config::set_active_provider(config, &provider, "")?;
        }
        config.delete("GOSLING_MODEL")?;
    } else {
...
pub async fn list_provider_secrets() -> Result<Json<ProviderSecretsResponse>, ErrorResponse> {
    let secrets = gosling::providers::provider_secrets::list_provider_secrets().await?;
...
pub async fn delete_provider_secret(Path(id): Path<String>) -> Result<Json<String>, ErrorResponse> {
    use gosling::providers::provider_secrets::DeleteProviderSecretError;

    match gosling::providers::provider_secrets::delete_provider_secret(&id).await {
...
pub async fn read_config(
    ...
    if query.key == "GOSLING_PROVIDER" || query.key == "active_provider" {
        if let Ok(val) = config.get_gosling_provider() {
            return Ok(Json(ConfigValueResponse::Value(Value::String(val))));
        }
        return Ok(Json(ConfigValueResponse::Value(Value::Null)));
    }
    if query.key == "GOSLING_MODEL" {
        if let Ok(val) = config.get_gosling_model() {
            return Ok(Json(ConfigValueResponse::Value(Value::String(val))));
        }
        return Ok(Json(ConfigValueResponse::Value(Value::Null)));
    }
...
pub async fn get_extensions() -> Result<Json<ExtensionResponse>, ErrorResponse> {
    let extensions = gosling::config::get_all_extensions()
        .into_iter()
        .filter(|ext| !gosling::agents::extension_manager::is_hidden_extension(&ext.config.name()))
        .collect();
    let warnings = gosling::config::get_warnings();
...
pub async fn add_extension(
    ...
    let extensions = gosling::config::get_all_extensions();
    let key = gosling::config::extensions::name_to_key(&extension_query.name);
...
pub async fn providers() -> Result<Json<Vec<ProviderDetails>>, ErrorResponse> {
    ...
    let providers_response: Vec<ProviderDetails> = providers
        ...
        .map(|(metadata, provider_type)| {
            ...
            let saved_model = gosling::config::get_provider_entry(config, &metadata.name)
                .map(|e| e.model)
                .filter(|m| !m.is_empty());
...
pub async fn get_provider_models(
    ...
    let provider = gosling::providers::create(&name, Vec::new()).await?;

    let models_result = provider
        .fetch_recommended_model_info(gosling::model_config::global_toolshim())
        .await;
...
pub async fn resolve_provider_model_info(
    ...
    let model_config = gosling::model_config::model_config_from_user_config(name, model)?;
    let provider = gosling::providers::create(name, Vec::new()).await?;
...
pub async fn get_slash_commands(
    ...
    for source in gosling::skills::list_installed_skills(working_dir.as_deref()) {
        commands.push(SlashCommand {
            command: source.name,
    ...
    for source in
        gosling::agents::platform_extensions::summon::discover_filesystem_sources(discover_dir)
    {
...
pub async fn create_custom_provider(
    ...
    let config = gosling::config::declarative_providers::create_custom_provider(
        gosling::config::declarative_providers::CreateCustomProviderParams {
    ...
    gosling::providers::refresh_custom_providers().await?;
...
pub async fn get_custom_provider(
    ...
    let loaded_provider = gosling::config::declarative_providers::load_provider(id.as_str())
        .map_err(|e| {
...
pub async fn remove_custom_provider(Path(id): Path<String>) -> Result<Json<String>, ErrorResponse> {
    gosling::config::declarative_providers::remove_custom_provider(&id)?;
...
pub async fn cleanup_provider_cache(
    ...
    gosling::providers::cleanup_provider(&name).await?;
...
pub async fn update_custom_provider(
    ...
    gosling::config::declarative_providers::update_custom_provider(
        gosling::config::declarative_providers::UpdateCustomProviderParams {
            id: id.clone(),
            engine: request.engine,
            display_name: request.display_name,
            api_url: request.api_url,
