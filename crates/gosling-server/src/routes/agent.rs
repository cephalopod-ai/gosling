...
async fn start_agent(
    ...
    gosling::posthog::set_session_context("desktop", false);
    ...
    let current_mode = config.get_gosling_mode().unwrap_or_default();
    ...
    let mut session = manager
        ...
        .map_err(|err| {
            ...
            #[cfg(feature = "telemetry")]
            gosling::posthog::emit_error("session_create_failed", &err.to_string());
            ErrorResponse {
...
async fn resume_agent(
    ...
    let session = state
        ...
        .map_err(|err| {
            ...
            #[cfg(feature = "telemetry")]
            gosling::posthog::emit_error("session_resume_failed", &err.to_string());
            ErrorResponse {
...
async fn update_agent_provider(
    ...
    let config = Config::global();
    let model = match payload.model.or_else(|| config.get_gosling_model().ok()) {
        Some(m) => m,
        None => {
    ...
    let mut model_config =
        gosling::model_config::model_config_from_user_config(&payload.provider, &model)
            .map_err(|e| {
                (
...
async fn update_session(
    ...
    if let Some(mode_str) = payload.gosling_mode {
        let mode: GoslingMode = mode_str.parse().map_err(|_| {
            (
                StatusCode::BAD_REQUEST,
