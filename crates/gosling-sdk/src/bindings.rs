//! In-process uniffi bindings for the Gosling SDK.
//!
...
//! infrastructure end to end without depending on the `gosling` core crate.
//!
//! To build the real SDK, add `gosling` (and whatever else you need) as
...
/// The top-level entry point for the Gosling SDK.
///
...
impl Client {
    ...
    pub fn ping(&self, message: String) -> Result<Pong, GoslingError> {
        if message.is_empty() {
            return Err(GoslingError::Generic("message must not be empty".into()));
        }
        Ok(Pong {
