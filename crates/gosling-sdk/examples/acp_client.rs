//! ACP Client Example
//!
//! Spawns `gosling acp` as a child process and sends it a completion request
//! using the Agent Client Protocol over stdio.
//!
//! # Prerequisites
//!
//! You must have gosling built and a provider configured (`gosling configure`).
//!
//! # Usage
//!
//! ```bash
//! cargo run -p gosling-sdk --example acp_client -- "What is 2 + 2?"
//! ```
//!
//! Or with a custom gosling binary path:
//!
//! ```bash
//! cargo run -p gosling-sdk --example acp_client -- --gosling-bin ./target/debug/gosling "Explain Rust's ownership model in one sentence"
//! ```
...
use gosling_sdk::custom_requests::GetExtensionsRequest;
use std::path::PathBuf;
use tokio_util::compat::{TokioAsyncReadCompatExt, TokioAsyncWriteCompatExt};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Parse args: [--gosling-bin PATH] PROMPT
    let args: Vec<String> = std::env::args().skip(1).collect();
    let (gosling_bin, prompt) = parse_args(&args)?;

    eprintln!("🚀 Spawning: {} acp", gosling_bin.display());

    let mut child = tokio::process::Command::new(&gosling_bin)
        .arg("acp")
        .stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::inherit())
        .spawn()
        .map_err(|e| format!("Failed to spawn '{}': {e}", gosling_bin.display()))?;
...
fn parse_args(args: &[String]) -> Result<(PathBuf, String), String> {
    let mut gosling_bin = PathBuf::from("gosling");
    let mut i = 0;
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--gosling-bin" => {
                i += 1;
                gosling_bin = PathBuf::from(args.get(i).ok_or("--gosling-bin requires a value")?);
            }
            _ => break,
        }
        i += 1;
    ...
    if prompt.is_empty() {
        return Err("Usage: acp_client [--gosling-bin PATH] PROMPT".into());
    }

    Ok((gosling_bin, prompt))
}
