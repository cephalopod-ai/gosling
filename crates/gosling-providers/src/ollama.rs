...
pub struct OllamaOptions {
    /// Explicit context window override from `GOSLING_INPUT_LIMIT`.
    ...
    /// `OLLAMA_STREAM_TIMEOUT` > `GOSLING_STREAM_TIMEOUT` > `OLLAMA_TIMEOUT` >
    /// default (120s).
...
impl OllamaProvider {
    ...
    async fn fetch_models_from_api(&self) -> Result<Vec<String>, ProviderError> {
        let response = self.api_client.request("api/tags").response_get().await?;
...
/// Configurable via OLLAMA_STREAM_TIMEOUT, GOSLING_STREAM_TIMEOUT, or falls back
                ...
                Err(_) => {
                    Err::<(), anyhow::Error>(anyhow::anyhow!(
                        "Ollama stream stalled: no data received for {}s. \
                         ...
                         timeout via OLLAMA_STREAM_TIMEOUT, GOSLING_STREAM_TIMEOUT, or \
...
mod tests {
    ...
    #[tokio::test]
    async fn fetch_supported_models_preserves_authentication_errors() {
        use wiremock::matchers::{method, path};
        use wiremock::{Mock, MockServer, ResponseTemplate};
