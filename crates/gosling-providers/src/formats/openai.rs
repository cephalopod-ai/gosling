...
pub fn create_request_with_options(
    ...
    // GOSLING_MAX_TOKENS) or a canonical model record has supplied a value.
...
mod tests {
    ...
    fn test_create_request_omits_max_tokens_when_unset() -> anyhow::Result<()> {
        ...
        // lmstudio) have no canonical record and no GOSLING_MAX_TOKENS, so the
...
data: [DONE]"#;
    ...
    fn test_extract_reasoning_effort_for_responses_models() {
        for (model, expected_name, expected_effort) in [
            ...
            ("gosling-gpt-5-high", "gosling-gpt-5", Some("high")),
            ("gpt-4o", "gpt-4o", None),
        ] {
