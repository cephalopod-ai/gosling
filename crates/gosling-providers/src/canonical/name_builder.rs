...
pub fn map_provider_name(provider: &str) -> &str {
    match provider {
        // Gosling provider names that differ from models.dev names
        "xai" => "x-ai",
        "azure_openai" => "azure",
...
mod tests {
    ...
    fn test_map_to_canonical_model() {
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "gosling-claude-4-sonnet-bedrock", r),
            Some("anthropic/claude-sonnet-4".to_string())
        );
        assert_eq!(
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "kgosling-gpt-4o", r),
            Some("openai/gpt-4o".to_string())
        );
        assert_eq!(
        ...
        // === OpenAI O-series ===
        assert_eq!(
            map_to_canonical_model("databricks", "gosling-o1", r),
            Some("openai/o1".to_string())
        );
        assert_eq!(
            map_to_canonical_model("databricks", "kgosling-o3", r),
            Some("openai/o3".to_string())
        );
        assert_eq!(
            map_to_canonical_model("databricks", "headless-gosling-o3-mini", r),
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "kgosling-grok-4.3", r),
            Some("x-ai/grok-4.3".to_string())
        );
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "gosling-command-r-08-2024", r),
